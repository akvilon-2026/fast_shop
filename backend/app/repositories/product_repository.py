...
class ProductRepository:
    ...
    def get_by_category(self,category_id: int)-> List[Product]:
        return self.db.query(Product).options(joinedload(Product.category)).filter(Product.category==category_id).all()

    def create(self,prodyct_data: ProductCreate)->Product:
        db_product=Product(**prodyct_data.model_dump())
